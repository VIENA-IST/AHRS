/*
 * RazorSettings.h
 *
 *  Created on: Dec 6, 2016
 *      Author: bruno
 */

#ifndef SRC_RAZOR_H_
#define SRC_RAZOR_H_

// #include <Arduino.h>
#include <ADXL345.h>
#include <ITG3200.h>
#include <HMC5843.h>
#include <stdint.h>

#ifndef DEBUG
#define DEBUG 0
#endif
#if DEBUG
#define DEBUG_MSG(...) Serial.println(__VA_ARGS__)
#endif
#ifndef DEBUG_MSG
#define DEBUG_MSG(...)
#endif

const uint8_t crc_table_maxim[256] PROGMEM = {};

class FastCRC8
{
  public:
	FastCRC8();
	~FastCRC8();
	uint8_t maxim(const uint8_t *data, const uint16_t datalen); // Equivalent to _crc_ibutton_update() in crc16.h from avr_libc
	uint8_t maxim_upd(const uint8_t *data, uint16_t datalen);   // Call for subsequent calculations with previous seed.
  private:
	uint8_t seed;
};

//! hold values of adxl345 calibration
struct AdxlSettings
{
	float ax_offset;
	float ay_offset;
	float az_offset;
	float ax_scale;
	float ay_scale;
	float az_scale;
};

//! hold values of itg3200 calibration
struct Itg3200Settings
{
	float gx_offset;
	float gy_offset;
	float gz_offset;
};

//! hold values of HMC5843
struct Hmc5843Settings
{
	float mx_offset;
	float my_offset;
	float mz_offset;
	float mTransform[3][3];
};

struct dataFrame
{
	uint8_t sync01 = 0xAA;
	uint8_t sync02 = 0xBB;
	uint8_t ID;
	float acc[3];
	float gyro[3];
	float mag[3];
	float euler[3];
	uint8_t crc8; //!maxim iButton CRC 8 bits
				  // more to be added
};

char *sprintFloat(float, char *);

class Razor
{
  private:
	FastCRC8 crc;
	const uint8_t packetSize = sizeof(this->outputPacket);
	float tempOutput_mag[3] = {0, 0, 0};

  public:
	struct dataFrame outputPacket;
	struct AdxlSettings accSettings;
	struct Itg3200Settings gyroSettings;
	struct Hmc5843Settings magSettings;
	uint16_t idxAcc;
	uint16_t idxGyro;
	uint16_t idxMag;
	Razor(const int idx = 0);
	~Razor();
	//* load settings from eeprom
	void loadAccSettings(const int);
	void storeAccSettings(const int);
	void loadMagSettings(const int);
	void storeMagSettings(const int);
	//* load ID from eeprom
	void loadID();
	void storeID();
	//* correct for scale, offsets, hard iron.
	void compensate_sensor_errors();
	//* print current values of sensor settings.
	void printAccSettings();
	void printGyroSettings();
	void printMagSettings();
	//* send binary data packet
	void sendPacket();
	void printPacket();

	const struct AdxlSettings &getAccSettings() const
	{
		return accSettings;
	}

	void setAccSettings(const struct AdxlSettings &accSettings)
	{
		this->accSettings = accSettings;
	}

	const struct Itg3200Settings &getGyroSettings() const
	{
		return gyroSettings;
	}

	void setGyroSettings(const struct Itg3200Settings &gyroSettings)
	{
		this->gyroSettings = gyroSettings;
	}

	uint8_t getId() const
	{
		return outputPacket.ID;
	}

	void setId(uint8_t id)
	{
		outputPacket.ID = id;
	}

	const struct Hmc5843Settings &getMagSettings() const
	{
		return magSettings;
	}

	void setMagSettings(const struct Hmc5843Settings &magSettings)
	{
		this->magSettings = magSettings;
	}
};

#endif /* SRC_RAZOR_H_ */
